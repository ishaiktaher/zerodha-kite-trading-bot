...
export async function GET(request) {
  ...
  const sql = getDb();
  const due = await sql`select id from zeta_gain.filters where is_active and run_daily_at is not null and run_daily_at <= localtime and not exists (select 1 from zeta_gain.scan_runs r where r.filter_id = filters.id and r.run_at::date = current_date)`;
